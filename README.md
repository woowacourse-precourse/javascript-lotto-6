...
- 세 개의 요구 사항을 만족시켜야 한다.
- 기능을 구현하기 전에 기능 목록을 만든다. 그 후 기능 단위로 커밋 하는 방식으로 진행한다.

- 기능 목록 만들기
- 기능 단위 커밋하기
- 구현하기 (프로그램 실행의 시작점은 `App.js`의 `play` 메서드이다. 아래와 같이 프로그램을 실행시킬 수 있어야 한다.)
  const app = new App();
  app.play();
- 구현완료하면 test
  npm test
- test 완료한 후 GitHub을 통해 제출해야 한다. (https://github.com/woowacourse/woowacourse-docs/tree/master/precourse)
