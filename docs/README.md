...
- [x] 시작 전 본인 깃허브 닉네임으로 된 브랜치를 만들고 체크아웃 하기
- [x] 체크아웃 이후 docs/README.md에 구현할 기능 목록을 정리해 추가하기
- [x] 구분없는 `let`, `const`를 사용하지 않기
- [x] 같은 변수에 값을 재할당하지 않기
- [x] 반복문에 의미없는 변수를 선언하였는지 확인하기 ex. `for(let i=0; i<n; i+=1)`
- [x] [커밋 메시지 컨벤션](https://gist.github.com/stephenparish/9941e89d80e2bc58a153)을 지켜 커밋 메세지 작성하기
...
- [x] README.md를 상세히 작성한다
  - [x] 지금 하는 프로젝트가 어떤 프로젝트인지 소개했는가?
  - [x] 어떤 기능을 담고 있는지 잘 나타내었는가?
- [x] 기능 목록을 재검토한다
  - [x] 너무 상세하게 작성하지 않도록 한다.
  - [x] 예외적인 상황도 정리했는가?
- [x] 기능 목록을 업데이트한다
  - [x] 기능을 구현하면서 문서를 계속 업데이트해나가고 있는가?
- [ ] 값을 하드 코딩하지 않는다
  - [x] 문자열, 숫자 등의 값들을 상수를 통해 관리하고 있는가?
- [x] 구현 순서도 코딩 컨벤션이다
  - [x] 클래스를 필드, 생성자, 메서드 순으로 작성했는가?
