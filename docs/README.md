...
- [ ] 2. 입력한 가격을 검증한다.
  - [ ] 2.1. 불필요한 공백 제거 처리
  - [ ] 2.2. 숫자만 입력했는지 확인. `true` -> `2.4`으로 이동. `false` -> `2.3`로 이동.
  - [ ] 2.3. _[ERROR] 숫자만 입력할 수 있습니다._ 에러를 출력하고 `1`로 돌아간다.
  - [ ] 2.4. 로또 1장당 1,000원이므로 1000단위로 입력했는지 확인. `true` -> `3`으로 이동. `false` -> `2.4`로 이동.
  - [ ] 2.5. _[ERROR] 로또는 1,000원 단위로만 구매할 수 있습니다.__ 에러를 출력하고 `1`로 돌아간다.
- [ ] 3. 로또 수량을 계산한다.
