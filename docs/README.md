...
- [ ] 생성한 로또에 대한 유효성 검사를 실시한다.
  - [ ] 6개의 서로 다른 숫자가 생성되지 않을 경우 예외처리
  - [ ] 1에서 45 사이의 정수가 생성되지 않을 경우 예외처리
- [ ] 예외 처리 시 [ERROR]로 시작하는 메시지를 출력하며 에러를 발생시킨다.
- [ ] 공백을 출력한다.
...
- 입력된 금액에 맞는 수량이 생성됐는지 검사
  ...
  - [ ] 입력 : 1000 출력 : lottos.length = 1

- 구매한 로또가 숫자 오름차순으로 정렬됐는지 검사

  - [ ] 입력 : [4,12,2,25,38,3] 출력 : [2,3,4,12,25,38](배열)
