...
- 예외를 발생시키는 경우는 아래와 같다.
  ...
  - 1부터 45까지의 숫자를 입력하지 않은 경우
  - 중복된 숫자가 있는 경우 (`Set`을 사용하면 개수로 예외 처리할 수 있다.)
  - 6개의 숫자를 입력하지 않은 경우
- ❗️ 에러 메시지를 출력하고 해당 부분부터 입력을 다시 받아야 한다.
...
- 발행된 한 개의 로또 단위로 당첨 번호와 비교하여 몇 개가 일치했는지 알아낸다.
- `filter()`를 사용한다.

### 당첨 통계 계산 기능

- 전체 발행된 로또 개수를 가지고 한 개씩 일치된 개수를 알아낸다.
- `일치된 개수를 알아내는 기능` 메서드를 사용한다.
- 일치된 개수를 가지고 카운트한다.
- 3개 이상 일치한 경우부터 카운트한다.

### 수익 금액과 수익률 계산 기능

- 3개 이상 일치한 경우부터 수익이 발생한다.
- 발행된 로또를 가지고 수익 금액을 합산한다.
...
- 발행된 로또 중 당첨 번호와 얼마나 일치됐는지의 정보: 객체
- 수익 금액: 숫자
- 수익률: 숫자(% 단위)
