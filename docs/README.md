...
- [x] 구입 금액을 입력 받는다.
  - [ ] (예외) 입력 값이 양의 정수가 아닌 경우 (ex. -1000, 0, 2.5, abcd)
...
- [ ] 당첨 번호를 입력받는다.
  ...
  - [ ] 입력 받은 당첨 번호를 쉼표(,)를 기준으로 구분하고, 각 번호의 앞뒤 공백을 제거한다.
- [ ] 보너스 번호를 입력받는다.
...
- [ ] `(로또 구입 금액 / 1000)`개 만큼 로또 발행을 반복한다.
  - [ ] (예외) 입력 값이 `1000`으로 나누어 떨어지지 않는 경우
- [ ] `6`개의 숫자를 번호로 갖는 로또를 발행한다.
  - [ ] (예외) 각 번호가 `1~45` 사이의 정수가 아닌 경우

    ```
    (예시) 0, -1, 2.5, 4 2, Hello
    ```

  - [ ] (예외) 번호의 총 개수가 `6`개가 아닌 경우
  - [ ] (예외) 서로 중복되는 번호가 있는 경우
- [ ] 발행한 로또의 번호를 오름차순으로 정렬한다.

### 당첨 로또 발행

- [ ] `6`개의 숫자를 당첨 번호로 갖는 당첨 로또를 발행한다.
  - [ ] (예외) 번호의 총 개수가 `6`개가 아닌 경우
  - [ ] (예외) 각 번호가 `1~45` 사이의 정수가 아닌 경우
  ...
  - [ ] (예외) 서로 중복되는 번호가 있는 경우
- [ ] 당첨 로또는 보너스 번호 `1`개를 추가적으로 갖는다.
  - [ ] (예외) 번호가 `1~45` 사이의 정수가 아닌 경우
...
- [ ] 로또 번호와 당첨 번호(+보너스 번호)를 비교하여 일치하는 번호의 개수를 센다.
...
- [ ] 수익률을 계산한다. `(수익률 = 당첨 금액의 합 / 로또 구입 금액 * 100)`
- [ ] 수익률을 소수점 둘째 자리에서 반올림하여 저장한다.
