...
describe('입력에 대한 유효성 test', () => {
  describe('구입금액에 대한 테스트', () => {
    ...
    test('1000보다 작은 금액을 입력했을 경우', () => {
      expect(() => {
        Validation.validatePurchaseAmount(999);
      }).toThrow(Error(ERROR_MESSAGE.invalidPusrchaseAmount));
    });

    test('1,000원으로 나누어 떨어지지 않는 경우', () => {
      expect(() => {
        Validation.validatePurchaseAmount(10001);
      }).toThrow(Error(ERROR_MESSAGE.invalidPusrchaseAmount));
    });
  });
  ...
  describe('당첨 번호에 대한 테스트', () => {
    test('유효한 당첨 번호를 입력했을 경우', () => {
      const result = Validation.validateWinningNumbers('1,2,3,4,5,6');
    ...
    test('당첨 번호를 6개 입력하지 않았을 경우', () => {
      ...
      }).toThrow(Error(ERROR_MESSAGE.duplicationWinningNumbers));
    });

    test('당첨 번호가 중복되었을 경우', () => {
      expect(() => {
        Validation.validateWinningNumbers('1,2,3,5,5,6');
      }).toThrow(Error(ERROR_MESSAGE.duplicationWinningNumbers));
    });

    test('당첨 번호에 소수가 포함되었을 경우', () => {
      expect(() => {
        Validation.validateWinningNumbers('1,2,3,3.5,5,6');
      }).toThrow(Error(ERROR_MESSAGE.duplicationWinningNumbers));
    });

    test('당첨 번호에 1~45사이가 아닌 숫자가 포함되었을 경우', () => {
      expect(() => {
        Validation.validateWinningNumbers('1,2,3,46,5,6');
      }).toThrow(Error(ERROR_MESSAGE.duplicationWinningNumbers));
    });
  });
