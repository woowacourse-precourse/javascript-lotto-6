...
describe("LottoView 클래스 테스트", () => {
  test("createPurchaseInfoString(): 정상적인 입력일 때 정확한 문자열을 반환한다.", () => {
    ...
    const formattedString = view.createPurchaseInfoString(
      numberOfSets,
      lottoNumbers
    );
  ...
  test("createStatisticsString(): 정상적인 입력일 때 정확한 문자열을 반환한다.", () => {
    const view = new LottoView();
    const statistics = {
      3: 0,
      4: 0,
    ...
    const formattedString = view.createStatisticsString(statistics);
    const expectedString =
