...
describe("Utils", () => {
  ...
  describe('addOrUpdatePropertyInObj 메소드', () => {
    test('객체에 속성을 추가 또는 업데이트 하고 반환', () => {
      const OBJ = { 2: 1, 4: 2 };
      const NEW_OBJ = Utils.addOrUpdatePropertyInObj(OBJ, 4);
      expect(NEW_OBJ).toEqual({ 2: 1, 4: 3 });
    });
  });
  ...
  describe('removeItemsWithNumericKeysLessThanThree 메소드', () => {
    test('숫자 키가 3보다 작은 항목을 객체에서 제거', () => {
      ...
      const NEW_OBJ = Utils.removeItemsWithNumericKeysLessThanThree(OBJ);
      expect(NEW_OBJ).toEqual({ 3: 4, 4: 5 });
    });
  });
  ...
  describe('addMissingElements 메소드', () => {
    test('WINNIG_PROFITS의 키와 일치하는 속성이 객체에 없으면 0으로 초기화하여 추가', () => {
      const OBJ = { 2: 1, 4: 2 };
      const NEW_OBJ = Utils.addMissingElements(OBJ);
      expect(NEW_OBJ).toEqual({ 2: 1, 4: 2, 3: 0, 5: 0,'bonus':0, 6: 0 }); // 예시 값에 따라 수정
    });
  });
});
