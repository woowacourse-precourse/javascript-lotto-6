...
class Lotto {
  ...
  findReward(winningNumbers, bonusNumber){
    const matchedNumbersCount = this.#numbers.filter(number => winningNumbers.includes(number)).length;
    const hasBonusNumber = this.#numbers.includes(bonusNumber);

    const rewardInfo = REWARD_INFOS.find(reward => reward.MATCH_COUNT === matchedNumbersCount && reward.BONUS === hasBonusNumber);
