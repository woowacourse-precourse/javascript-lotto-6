...
class LottoGame {
  ...
  async start() {
    ...
    const purchaseLottos = lottos.map((lotto) => new Lotto(lotto));

    // 구매 내역 출력
    Print.purchaseLottos(lottos);
    ...
    // 각 등수별 당첨 횟수 구하기
    purchaseLottos.forEach((lotto) => this.#addWinning(lotto, answerNumbers, bonusNumbers));
