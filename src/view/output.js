...
import { GAMEMSG_OUTPUT_BOUGHT_LOTTO_NUMBERS, GAMEMSG_OUTPUT_RESULT } from "../constants/GameMessage.js";
...
class Output{
    ...
    static outputScoreBoard(scoreBoard){
        ...
        [3,4,5,5.5,6].forEach((scores)=>{
            ...
            if(scores !== 5.5) Console.print(`${scores}개 일치 (${PRIZE_MONEY[place].toLocaleString('ko-KR')}원) - ${scoreBoard[place]}개`);
            if(scores === 5.5) Console.print(`5개 일치, 보너스 볼 일치 (${PRIZE_MONEY[place].toLocaleString('ko-KR')}원) - ${scoreBoard[place]}개`);
        })
    }

    static outputRevenuePercent(revenuePercent){
        Console.print(`총 수익률은 ${revenuePercent.toLocaleString('ko-KR')}%입니다.`);
    }
}
