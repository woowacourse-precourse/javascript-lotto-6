...
class App {
  async play() {
    const BUY_LOTTO = async() => {
      ...
      CREATE_WINNING_NUMBERS(ARRAY_LOTTO); // 로또 목록 전달
    }

    const CREATE_WINNING_NUMBERS = async(ARRAY_LOTTO) => {
      ...
      RESULT(ARRAY_LOTTO, WINNING_NUMBERS, BONUS_NUMBER); // 로또 목록, 당첨 및 보너스 번호 전달
    }

    const RESULT = async(ARRAY_LOTTO, WINNING_NUMBERS, BONUS_NUMBER) => {
      const LOTTO_RESULT = [
