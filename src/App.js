...
class App {
    ...
    async inputCashAmount() {
        while (true) {
            try {
                const cashAmount = await Console.readLineAsync("구입금액을 입력해 주세요.");
                new Buying(cashAmount);
                return cashAmount;
            } catch (error) {
    ...
    async inputWinningNumbers() {
      while (true) {
          try {
              const winningNumbers = await Console.readLineAsync("당첨 번호를 입력해 주세요.").then((numbers) => numbers.split(",").map(Number));
    ...
    async inputBonusNumber(winningNumbers) {
      while (true) {
          try {
              ...
              new WinningNumber(winningNumbers, bonusNumber);
              return bonusNumber;
          } catch (error) {
              Console.print(error.message);
          }
    ...
    generateLottoTickets(buying) {
        const ticketCount = buying.getAmount() / 1000;
        const lottoTickets = [];
    ...
    calculateTotalProfit(matchingCounts, cashAmount) {
    ...
    async play() {
      const buying = new Buying(await this.inputCashAmount());
      Console.print(buying.getAmount() / 1000 + "개를 구매했습니다.");

      const lottoTickets = this.generateLottoTickets(buying);
      ...
      this.calculateTotalProfit(matchingCounts, buying.getAmount());
  }
}
