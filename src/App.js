...
class App {
  async play() {
    ...
    const inputWinnerLotto = await Console.readLineAsync(GAME_MESSAGE.promptWinnerLotto);
    Console.print(inputWinnerLotto);
    const lotto = new Lotto(inputWinnerLotto.split(',').map((num) => Number(num)));
    ...
    await new Game(purchasedLottoList, winnerLotto, bonusNumValidator.bonusNum, purchaseAmount);
  }
}
