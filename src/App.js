...
class App {
  ...
  async lottoMainNumbers() {
    const lottoMainNumberInput = await Console.readLineAsync('\n당첨 번호를 입력해 주세요. \n');
  ...
  async lottoBonusNumber(lottoMainNumbers) {
    const lottoBonusNumberInput = await Console.readLineAsync('\n보너스 번호를 입력해 주세요. \n');
  ...
  printResult(lottoPrice, lottoTickets, calculationResult) {
    ...
    const totalPrice = resultPrice - lottoPrice;
    const totalPercentage = (100 + (totalPrice / lottoPrice) * 100).toFixed(1);
