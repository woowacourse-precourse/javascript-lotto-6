...
import { generateRandomNumbers, getEarnings, getScoreKey, calculatePercentageProfit } from './utilities.js';
...
class Vendor {
    ...
    #calculateEarnings(matches) {
        ...
        for (let i = 0; i < matchingKeys.length; i += 1) {
          const amountEarned = getEarnings(matchingKeys[i]) * matches[matchingKeys[i]];
